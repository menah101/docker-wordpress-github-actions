<?php
/**
 * bs2wooo functions and definitions
 *
...
function bs2wooo_setup() {
	...
	/*
		...
		* If you're building a theme based on bs2wooo, use a find and replace
		* to change 'bs2wooo' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'bs2wooo', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'bs2wooo' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'bs2wooo_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'bs2wooo_setup' );
 ...
 */
function bs2wooo_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'bs2wooo_content_width', 640 );
}
add_action( 'after_setup_theme', 'bs2wooo_content_width', 0 );
 ...
 */
function bs2wooo_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'bs2wooo' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'bs2wooo' ),
...
add_action( 'widgets_init', 'bs2wooo_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function bs2wooo_scripts() {
	wp_enqueue_style( 'bs2wooo-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'bs2wooo-style', 'rtl', 'replace' );

	wp_enqueue_script( 'bs2wooo-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'bs2wooo_scripts' );
